AFRAME.registerComponent('frame', {
  ...
  init: function() {
    ...
    function addFrameOccluder() {
      ...
      const OFFSET = (FRAME_EL) ? 0.01 : 0

      parts[0].object3D.position.set(-(HALF_WIDTH + HALF_SCALE) + OFFSET, 0, 0)
      parts[1].object3D.position.set(HALF_WIDTH + HALF_SCALE + OFFSET, 0, 0)
      parts[2].object3D.position.set(0, 0, -(HALF_HEIGHT + HALF_SCALE) + OFFSET)
      parts[3].object3D.position.set(0, 0, HALF_HEIGHT + HALF_SCALE + OFFSET)
    }
